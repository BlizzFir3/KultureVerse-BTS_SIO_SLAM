...
$questions 		= getQuestion($idQuestion);
?>
					...
					<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
						<form method="post">
							<div class="mb-3">
								...
								<textarea class="form-control" name="question" required><?= $aQuestion->question ?></textarea>
							</div>
							<?php
							if ($aQuestion->reponse == 1) {
								echo '<input type="radio" class="btn-check" name="answer" id="yes" autocomplete="off" value="true" checked><label class="btn btn-outline-success" for="yes">Oui</label><input style="margin-left: 1.3em;" type="radio" class="btn-check" name="answer" id="no" autocomplete="off" value="false"><label style="margin-left: 1.3em;" class="btn btn-outline-danger" for="no">Non</label><br />';
							} else {
								echo '<input type="radio" class="btn-check" name="answer" id="yes" autocomplete="off" value="true"><label class="btn btn-outline-success" for="yes">Oui</label><input style="margin-left: 1.3em;" type="radio" class="btn-check" name="answer" id="no" autocomplete="off" value="false" checked><label style="margin-left: 1.3em;" class="btn btn-outline-danger" for="no">Non</label><br />';
							}
							?>
							<button style="margin-top: 1.3em;" type="submit" name="submit" class="btn btn-warning">Modifier la question</button>
						</form>
					</div>
				<?php endforeach; ?>
			</div>
		...
		if (isset($_POST['answer']) && isset($_POST['question'])) {
			...
			if (!empty($_POST['answer']) && !empty($_POST['question'])) {
				...
				try {
					...
					editQuestion($idQuestion, $question, $reponse);
